            ...
            if (checkbrute($user_id, $mysqli) == true) {
				...
				if (empty($error_msg)) {
					if ($stmtbrut = $mysqli->prepare("UPDATE etsim_members SET enable_etsim_members = '0' WHERE id_etsim_members = ?")) {
						...
						$message = "The account $username (email: $email) has been disabled due to a suspicion of brute force attack.
									Connect to ETSIM to change the status of this user.
											";
						$headers = "From: etsim.SERIOUS-GAME@utbm.fr";
						$email_to = "ahmet.imre@utbm.fr";
						$subject = "ETSIM member disabled (brute force attack)";
...
function createTable($mysqli, $sql_query) {
  ...
  while ( $row = array_shift( $result ) ) {
    ...
    echo $row['id_etsim_members'];
		echo '</td><td>';
		echo $row['username_etsim_members'];
		echo '</td><td>';
		echo $row['email_etsim_members'];
		echo '</td><td>';
		echo $row['group_etsim_members'];
		echo '</td><td>';
    ...
    $resulttableSelect = get_result($stmttableSelect);
		#while($rowresulttableSelect = $resulttableSelect->fetch_assoc()) {
    while ( $rowresulttableSelect = array_shift( $resulttableSelect ) ) {
		...
		echo '</td><td>';
		echo '<button type="submit" class="delete_etsim_members">Delete</button>';
		echo '</td></tr>';
	}
}
?>
